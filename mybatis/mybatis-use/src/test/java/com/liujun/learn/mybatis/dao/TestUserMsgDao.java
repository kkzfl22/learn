...
public class TestUserMsgDao {
  ...
  public void baseQuery() {
    ...
    try (SqlSession session = sessionFactory.openSession(); ) {
      ...
      // 4, 执行一个查询
      List<UserMsgPO> queryList = session.selectList("UserMsg.selectAll");
  ...
  public void baseInsert() {
    ...
    try (SqlSession session = sessionFactory.openSession(); ) {
      ...
      // 4, 执行一个添加操作
      int insert = session.insert("UserMsg.insertValue", userDataSave);
      session.commit();
  ...
  public void baseUpdate() {
    ...
    try (SqlSession session = sessionFactory.openSession(); ) {
      ...
      // 4, 执行一个修改操作
      int update = session.update("UserMsg.update", userUpdate);
      session.commit();
  ...
  public void baseDelete() {
    ...
    try (SqlSession session = sessionFactory.openSession(); ) {
      ...
      // 4, 执行一个删除操作
      int delete = session.delete("UserMsg.delete", 5);
      session.commit();
